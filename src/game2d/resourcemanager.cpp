...
Shader *ResourceManager::loadShaderFromFile(const GLchar *vShaderFile, const GLchar *fShaderFile, const GLchar *gShaderFile)
{
    ...
    try
    {
        ...
        if (!vertexShaderFile.is_open()) {
            std::cout << "vshaderfile open faile" << vShaderFile << std::endl;
        }
        if (!fragmentShaderFile.is_open()) {
            std::cout << "fragmentShaderFile open faile" << fShaderFile << std::endl;
        }
...
Texture *ResourceManager::loadTextureFromFile(const GLchar *file, GLboolean alpha)
{
    ...
    unsigned char* image = stbi_load(file, &width, &height, &chanel ,0);
